...
# Compare insights from variable importance analyses here and the corresponding results from the Investigation 
...
We want a combination of both, our spline model has the lowest RMSE but could be difficult to interpret. We need more visualization and analysis before we move forward. However we are confident in moving forward.

# Societal Impact

Being that the data here is about movies, the harm that could come from our analysis would be minimal. 
Representation of different races, genders, abilities, etc. is an issue in Hollywood/the greater film industry, and it's likely that the harm our model/analysis produces is already in line with the issues that created and maintain this.
This is because the data we have is on past movies that have come out of this system. This means that whatever has made a movie successful, whether it's due to certain actors or directors, or whatever — this is a standard that has already existed. If we see the movies that have done really well because the cast is predominantly white, well this has been going on a for a long time because Hollywood is racist. Our model can't account for that, but we are aware of it. 
